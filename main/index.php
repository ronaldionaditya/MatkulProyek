...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['approve'])) {
        ...
        $updateQuery = "UPDATE requestbarang SET status_submit = 2 WHERE idRequest = '$idRequest'";
        mysqli_query($conn, $updateQuery);
    } elseif (isset($_POST['reject'])) {
        $idRequest = $_POST['idRequest'];
        $updateQuery = "UPDATE requestbarang SET status_submit = 3 WHERE idRequest = '$idRequest'";
        mysqli_query($conn, $updateQuery);
    }
    header("Location: index.php"); // Redirect untuk menghindari resubmission
    exit();
}

// Ambil data dari tabel requestbarang dengan status_submit = 1
$query = "SELECT idRequest, kodeBarang, namaBarang, jumlah, alasan, tanggalRequest, status_submit FROM requestbarang WHERE status_submit = 1 ORDER BY tanggalRequest DESC";
                            ...
                            <div class="table-responsive">
                                <table id="datatablesSimple" class="table table-striped table-bordered">
                                    ...
                                    <tbody>
                                        ...
                                        <td>
                                        <button class="btn btn-primary btn-sm" data-bs-toggle="modal" data-bs-target="#modalDetail<?php echo $row['idRequest']; ?>">Detail</button>
                                        </td>
                                        </tr>
                                    <!-- Modal Detail -->
